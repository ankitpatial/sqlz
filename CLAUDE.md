...
```sh
zig build                          # Build the executable (output: zig-out/bin/zqlc)
...
zqlc is a CLI code generator inspired by [sqlc](https://sqlc.dev). It reads `.sql` files, introspects queries against a live PostgreSQL database, and generates type-safe Zig source code.
...
- **Generated code marker** — all output files start with `// Code generated by zqlc. DO NOT EDIT.`
